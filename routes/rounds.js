...
router.put('/:game_id', function(req, res) {
    ...
    // Grab data from http request
    var data = {complete: req.body.complete,
        total_item_value: req.body.total_item_value,
        total_num_items: req.body.total_num_items,
        item_witheld: req.body.item_witheld,
        players: req.body.players
    ...
    pg.connect(connectionString, function(err, client, done) {
        ...
        client.query("UPDATE rounds SET complete=($1), total_item_value=($2), total_num_items=($3), item_witheld=($4), players=($5) WHERE game_id=($6)",
            [data.complete, data.total_item_value, data.total_num_items, data.item_witheld, data.players, id]
        );
